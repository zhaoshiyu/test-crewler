...
def crawler(main_url):
    ...
    while queue:  #and count <= 500000
        ...
        link_regex = re.compile('href="(http://finance\.qq\.com/a/\d+/\d+\.htm)"')
        for link in link_regex.findall(data):
            if link not in visited and link not in queue:
                print(link)
                queue.append(link)
                print('加入队列 --->  ' + link)
        link_regex = re.compile('href="(http://stock\.qq\.com/a/\d+/\d+\.htm)"')
        for link in link_regex.findall(data):
            if link not in visited and link not in queue:
                print(link)
                queue.append(link)
                print('加入队列 --->  ' + link)
        link_regex = re.compile('href="(http://money\.qq\.com/a/\d+/\d+\.htm)"')
        for link in link_regex.findall(data):
            if link not in visited and link not in queue:
                print(link)
                queue.append(link)
